...
BEGIN_MESSAGE_MAP(CTestDlg, CDialogEx)
	...
	ON_BN_CLICKED(IDC_BUTTON1, &CTestDlg::OnBnClickedCali)
END_MESSAGE_MAP()
...
void mat2Ply(Mat image)
{
	...
	strcpy(textureFile, "c_c_rgb");
	strcat(textureFile, s_number);
	strcat(textureFile, ".jpg");
	...
	for (y = 0; y < image.rows - 1; y++)
	{
		...
		{
			z = image.at<unsigned short>(y, x);
			if (/*z > 0*/z > USHRT_MAX / 320 && z < USHRT_MAX / 40)
			{
				// 텍스쳐 위치 관련은 여기서. u--일수록 텍스쳐가 오른쪽(열었을 때)으로 밀림. 
				// Texture coordinate가 아주 정확한 건 아님. 마음에 안 들면 4, 5번째 %f를 변경하길 바람.
				...
				sprintf(temp, "%f %f %f %f %f\n",
					(float)x / 300 / MODEL_SCALE, -(float)y / 300 / MODEL_SCALE, -(float)z / USHRT_MAX * 150,/*X*/ 0.05400 + ((float)(x + 1)) / (DEPTH_WIDTH * MODEL_SCALE) * 3 / 4, /*Y*/-0.0250 - ((float)(y + 1)) / (DEPTH_HEIGHT * MODEL_SCALE));
				content += temp;
