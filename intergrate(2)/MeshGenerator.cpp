...
#define MODEL_WIDTH 480*8
#define MODEL_HEIGHT 360*8
...
void mat2Ply(Mat image)
{
	...
	memset(vtrArr, 0, sizeof(Vertice)* MODEL_WIDTH * MODEL_HEIGHT);
		...
		for (x = 0; x < image.cols; x++)
		{
			...
			if (z > 0 && z < USHRT_MAX / 80)
			{
				...
				sprintf(temp, "%f %f %f %f %f\n",
					(float)x / 300, -(float)y / 300, -(float)z / USHRT_MAX * 180, 0.04828 + ((float)(x + 1)) / MODEL_WIDTH * 3 / 4, -0.0101 - ((float)(y + 1)) / MODEL_HEIGHT);
				content += temp;
...
int main(void)
{
	...
	while (1)
	{
		...
		if (doSave == true)
		{
			...
			cv::resize(depthMat, temp, cv::Size(depthMat.cols * 8, depthMat.rows * 8));
